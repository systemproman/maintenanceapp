...
def criar_usuario(funcionario_id: str = None, nome: str = None, email: str = None, username: str = None,
    ...
    cur.execute(
        ...
        (
            ...
            _upper(nivel_acesso) or 'VISUALIZACAO',
            1 if ativo else 0,
    ...
    try:
        registrar_log_acao('CRIAR', 'USUARIO', usuario_id, {
            ...
            'nivel_acesso': _upper(nivel_acesso),
...
def criar_usuario(funcionario_id: str = None, nome: str = None, email: str = None, username: str = None,
    ...
    cur.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
        """,
        (usuario_id, funcionario_id or None, nome_exibicao, email, username, _hash_password(senha_placeholder), _upper(nivel_acesso) or 'VISUALIZACAO', 1 if ativo else 0, 1 if pode_ver_logs else 0),
    )
    conn.commit()
    email_enviado = False
    ...
    if enviar_email:
        try:
            ...
            email_enviado = False
    try:
        registrar_log_acao('CRIAR', 'USUARIO', usuario_id, {'username': username, 'nome': nome_exibicao, 'email': email, 'tipo': 'FUNCIONARIO' if funcionario_id else 'EXTERNO', 'nivel_acesso': _upper(nivel_acesso), 'pode_ver_logs': bool(pode_ver_logs), 'email_enviado': email_enviado, 'link_redefinicao_gerado': bool(link_redefinicao)})
    except Exception:
        pass
...
def _migrar_perfis_v2():
    """
    1. Garante que a coluna nivel_acesso aceite os novos valores.
    2. Converte perfis legados: COMPLETO → GERENCIA, VISUALIZACAO → TECNICO.
    3. Eleva o usuário 'admin' para ADMIN.
    Roda em cada start — idempotente.
    """
    cur = _cursor()
    try:
        # Converte COMPLETO → GERENCIA
        cur.execute(
            "UPDATE usuarios SET nivel_acesso = 'GERENCIA', updated_at = CURRENT_TIMESTAMP "
            "WHERE UPPER(COALESCE(nivel_acesso,'')) = 'COMPLETO'"
        )
        # Converte VISUALIZACAO → TECNICO
        cur.execute(
            "UPDATE usuarios SET nivel_acesso = 'TECNICO', updated_at = CURRENT_TIMESTAMP "
            "WHERE UPPER(COALESCE(nivel_acesso,'')) = 'VISUALIZACAO'"
        )
        # admin sempre ADMIN
        cur.execute(
        ...
        print(f'[WARN] _migrar_perfis_v2: {_me}')


try:
    _migrar_perfis_v2()
except Exception as _me2:
    print(f'[WARN] migração perfis: {_me2}')
